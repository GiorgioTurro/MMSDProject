...
def update_hospital_capacity(old_hosp_list, new_hosp_list):
    ...
    # Se un reparto chiude dove metto la gente in fila?
    return new_hosp_list
...
def start_simulation(resources, patients, hosp_dict):
    ...
    # Creo il file di log
    save_info.create_day_log(patient_day_list, hosp_list)
    ...
    for day in patient_day_list:
        ...
        if new_year != old_year:
            old_year = new_year
            new_hosp_list = create_hospital_list(hosp_dict, old_year)
        ...
        save_info.save_day_info(hosp_list, d)
...
"""
    # Conto quanti giorni ci sono (per adesso valore non usato)
    date0 = (str(patients['data_ricovero'].iloc[0])).split(" ")[0]
    d0 = date(int(date0.split("-")[0]), int(date0.split("-")[1]),
              int(date0.split("-")[2]))
    date1 = (str(patients['data_ricovero'].iloc[-1])).split(" ")[0]
    d1 = date(int(date1.split("-")[0]), int(date1.split("-")[1]),
              int(date1.split("-")[2]))
    delta = (d1-d0).days
"""
